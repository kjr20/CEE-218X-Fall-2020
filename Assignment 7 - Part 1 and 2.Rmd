...
The first part of the assignment focused on vehicles emissions in Sebastopol, 
...
For this assignment Safegraph data will be explored because device data offers 
a better understanding of the commutes people make not just to work. People make 
commutes in their every day lives, such as to the grocery store, and those are 
not accounted for through LODES data. 

Loaded necessary packages including the Safegraph Neighborhood Patterns data:
...
Loaded CA census block group data and then filtered them to Sebastopol, CA
which is the focus area:
...
JSON data counts the number of trips made by devices whose home locations are
not in the CBG of Sebastopol. This grabs JSON strings and converts them 
to a dataframe:
...
JSON doesn't give a dataframe that we can use immediately so this loops 
through the rows of the original dataframe and converts device counts from
"4" to "3" to more accurately average the options of 2 to 4 devices:
...
This merges decvices that might have been a single trip but were stopped. This 
makes sure that trips are not double counted:
...
This data sheet now normalizes the total number of devices SafeGraph monitors 
in an origin CBG (Sebastopol):

```{r}
seb_home_panel_summary <- read_csv("home_panel_summary.gz")
```

Trips to Sebastopol may come form other places, but we will set them to car
trips for the sake of understanding emissions:
...
Filters the number of trips with origins that will not be considered because
they are out of state / too far away to be a car trip:
...
Normalized the number of devices residing in Sebastopol to the population:
...
Next, obtaining the data of travel distances for each origin CBG to Sebastpol: 
...
This accounts for the number of other visits that were counted by SafeGraph
but are not taken into account in the calculation of only commute trips.
A ratio is outputted suggesting that the number of trips that are acutally 
missed by using only commute trips and trips that are in state: 
...
This refines the total number of vehicles traveled by dividing the vehicle miles
traveled calculation using commute trips by ratio of potential trips missed in
the calculation: 
...
Executes 2020 Summer vehicle EMFAC data obtained from the California Air 
Resources Board: 
...
Calculates the emissions produced by commuters to Sebastopol, CA:
...
energy use by parcel for Sebastopol, CA in Sonoma County. I found open data
from the Sonoma County site that divides parcels into residential and commercial
parcel catergories. I used 2020 data that assumes an emission factor of 210 
(PGE). I used 2020 data to match SafeGraph data used in part 1.

Inputs PG&E data for the year 2020:
...
Filters energy data for just the month of June 2017:
...
Creates a map of June 2017 energy use in Sebastopol:
...
ggplot(
  ...
  geom_bar(stat = "identity", aes(fill = UseCodType), position = "dodge") + 
  labs(x = "Month", y = "GBTU", title = "Sebastopol Energy Usage, June 2017") + 
  scale_fill_discrete(name="Energy Type",labels = c("Elec-Commercial",
                                                    "Elec-Residential",
                                                    "Gas-Commercial",
                                                    "Gas-Residential"))
```

Creates a map of June 2017 emissions in Sebastopol:
...
ggplot(
  ...
  geom_bar(stat = "identity", aes(fill = UseCodType), position = "dodge") + 
  labs(x = "Month", y = "tCO2e", title = "Sebastopol Emissions, June 2017") + 
  scale_fill_discrete(name="Energy Type",labels = c("Elec-Commercial",
                                                    "Elec-Residential",
                                                    "Gas-Commercial",
                                                    "Gas-Residential"))
...
available parcel data may be requested often, but I operated under the 
assumption that parcels have not changed in 2017 for the purpose of creating
an energy analysis. 
...
Filters the parcels to only residential building area:
...
Filters the parcels to only commercial building area:
...
Filters the total residential parcel data to square feet:

```{r}
seb_parcel_final_res <-
seb_parcels_res%>%
summarize(
...
```{r}
elec_com_use_per_sqft = seb_pge_june_2017[1,2] / seb_parcel_final_com[1,1]
```
...
```{r}
elec_com_emissions_per_sqft = seb_pge_june_2017[1,3] / seb_parcel_final_com[1,1]
```
...
```{r}
elec_res_use_per_sqft = seb_pge_june_2017[2,2] / seb_parcel_final_res[1,1]
```
...
```{r}
elec_res_emissions_per_sqft = seb_pge_june_2017[2,3] / seb_parcel_final_res[1,1]
```
...
```{r}
gas_com_use_per_sqft = seb_pge_june_2017[3,2] / seb_parcel_final_com[1,1]
```

Total commercial gas emissions per parcel square feet in Sebastopol for the 
month of June 2017: 

```{r}
gas_com_emissions_per_sqft = seb_pge_june_2017[3,3] / seb_parcel_final_com[1,1]
```
...
```{r}
gas_res_use_per_sqft = seb_pge_june_2017[4,2] / seb_parcel_final_res[1,1]
```
...
```{r}
gas_res_emissions_per_sqft = seb_pge_june_2017[4,3] / seb_parcel_final_res[1,1]
```

The total commercial electricity use in Sebastopol per parcel square foot for 
the month of June 2017 is 0.67991 kBTU. 
The total commercial electricity emissions in Sebastopol per parcel square 
foot for the month of June 2017 is 0.0001806427 tCO2E. 

The total residential electricity use in Sebastopol per parcel square foot for 
the month of June 2017 is 0.8678829 kBTU. 
The total residential electricity emissions in Sebastopol per parcel square 
foot for the month of June 2017 is 1.834282e-05 tCO2E.

The total commercial gas use in Sebastopol per parcel square foot for 
the month of June 2017 is 0 kBTU. 
The total commercial gas emissions in Sebastopol per parcel square 
foot for the month of June 2017 is 0 tCO2E.

The total residential gas use in Sebastopol per parcel square foot for 
the month of June 2017 is 0.4054395 kBTU. 
The total residential gas emissions in Sebastopol per parcel square 
foot for the month of June 2017 is 2.153401e-05 tCO2E.


Sources Used:

https://docs.safegraph.com/docs/home-panel-summary
